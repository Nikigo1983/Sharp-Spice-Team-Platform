...
const OUT_DIR = path.join(process.cwd(), "reports", "pr7-crud-ui");
...
async function main() {
  ...
  await page.waitForSelector("text=Новое событие", { timeout: 30_000 });
  await page.screenshot({
    path: path.join(OUT_DIR, "01-create-modal.png"),
    fullPage: true,
  });
  ...
  await page.getByRole("button", { name: /Консультация с клиентом/ }).click();
  await page.waitForSelector("text=Редактировать", { timeout: 30_000 });
  await page.screenshot({
    path: path.join(OUT_DIR, "02-view-modal.png"),
    fullPage: true,
  });
