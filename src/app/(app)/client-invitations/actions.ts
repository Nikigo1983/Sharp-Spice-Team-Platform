...
import {
  ...
  deleteClientInvitationForStaff,
...
export async function deleteClientInvitationAction(input: {
  invitationId: string;
  ...
  if (!invitationId) {
    return { ok: false, error: "Не указано приглашение." };
  }

  try {
    await deleteClientInvitationForStaff(invitationId);
    return { ok: true };
  } catch (error) {
    console.error("[client-portal] delete invitation", error);
    const message = error instanceof Error ? error.message : "";
    if (message === "NOT_FOUND") {
      return { ok: false, error: "Приглашение уже удалено или не найдено." };
    }
    return {
      ok: false,
