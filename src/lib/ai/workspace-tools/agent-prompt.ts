...
3. После get_client / get_case_context опирайся на возвращённые поля (email, phone, status, contract*, Finance). Паспорт / DOB / адрес по умолчанию не приходят в tool payload — не выдумывай их и не проси «полный сырой ряд».
...
22. «Напиши письмо X оплатить долг / ВНЖ» — составь готовый деликатный текст клиенту по Finance (сумма, email). Не отказывайся из‑за длины чата.
`.trim();
